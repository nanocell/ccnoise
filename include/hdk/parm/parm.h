...
namespace cchdk
{
	...
	template<std::size_t N, class DefaultsT=NoDefaults, class RangeT=NoRange>
	struct Parm
	{
		static const std::size_t elements = N;
		Parm(PRM_Type prm_type, std::string aname, std::string alabel, DefaultsT adefaults=DefaultsT(), RangeT arange=RangeT()) :
			...
			prm_range(arange)
		{
			...
			tpl = PRM_Template(prm_type, this->elements, &(this->prm_name), this->prm_defaults.get(), 0, this->prm_range.get() );
		}
